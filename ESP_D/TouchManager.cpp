...
void handleTouch() {
  if (millis() - lastAnalogReadTime < ANALOG_READ_DELAY)
    return;
  lastAnalogReadTime = millis();
  bool currentState =
      digitalRead(TOUCH_PIN) ? HIGH : LOW;
  ...
  if (touchPhase == TOUCH_COUNTING && currentState == LOW &&
    ...
    if (touchCount == 1) {
      toggleServo(0);
      log("TOUCH", "1 tap → toggleServo1 (den chinh)");
    } else if (touchCount == 2) {
      safePub("espE/servo1/set", "TOGGLE");
      log("TOUCH", "2 tap → toggle ESP_E Servo 1");
    } else if (touchCount == 3) {
      toggleRelay(0);
      log("TOUCH", "3 tap → toggle quat oi nuoc");
    } else if (touchCount == 4) {
      toggleRelay(1);
      log("TOUCH", "4 tap → toggle quat cay");
    } else {
      logf("TOUCH", "No action for count=%d", touchCount);
    }
